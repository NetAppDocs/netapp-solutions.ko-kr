...
[%collapsible]
====
사전 요구 사항이 용량 할당된 상태에서 EC2 인스턴스에 루트 사용자로 로그인하여 Oracle 설치를 위한 Linux 커널을 구성합니다.
...
[%collapsible]
====
FSx 콘솔에서 Oracle 데이터베이스 바이너리, 데이터 및 로그 파일을 호스팅하도록 세 개의 볼륨을 프로비저닝합니다.
...
. EC2 사용자로 SSH를 통해 EC2 인스턴스에 로그인합니다. 그런 다음 SSH 키와 EC2 인스턴스 IP 주소로 변경합니다.
+
....
...
....
. 장치 소유권을 로 변경합니다 `oracle:oinstall`.
+
....
...
....
. Oracle 사용자로 를 시작합니다 `gridSetup.sh` 그리드 인프라 설치용.
+
....
...
....
. "[INS-43080] 일부 구성 도우미가 실패했거나, 취소되었거나, 건너뛰었습니다" 메시지를 받은 경우 Oracle 사용자로 다음 명령을 실행하여 데이터 디스크 그룹 리소스가 생성되었는지 확인하십시오.
+
....
bin/crsctl stat res -t
....
. 데이터 디스크 그룹이 생성되지 않은 경우 multipathd를 루트 사용자로 다시 로드합니다.
+
....
systemctl restart multipathd
....
. Oracle 사용자는 그리드 설치 중에 데이터 디스크 그룹이 생성되지 않은 경우 데이터 디스크 그룹을 생성합니다.
+
....
...
....
. 소프트웨어 전용 DB 자동 설치 실행
+
....
 ./runInstaller -applyRU /tmp/archive/34765931/ -silent -ignorePrereqFailure -responseFile /tmp/archive/dbinstall.rsp
....
. 를 실행합니다 `root.sh` 소프트웨어 전용 설치 후 스크립트.
+
....
/u01/app/oracle/product/19.0.0/db1/root.sh
....
. 를 생성합니다 `dbca.rsp` 다음 항목이 있는 파일:
+
....
...
....
. DB 생성 후 Oracle Restart HA 서비스를 확인합니다.
+
....
