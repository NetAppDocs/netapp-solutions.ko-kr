...
image::storage_layout_sqlsvr_large.PNG[스토리지 레이아웃 sqlsvr large]

다음 그림은 LUN의 중소 규모 SQL Server 데이터베이스에 대한 NetApp 권장 스토리지 레이아웃을 보여 줍니다.

image::storage_layout_sqlsvr_smallmedium.PNG[스토리지 레이아웃 sqlsvr smallmedium]
...
image::storage_layout_oracle_typical.PNG[스토리지 레이아웃 Oracle 일반]
...
image::storage_layout_oracle_dedicated.PNG[스토리지 레이아웃 Oracle 전용]
...
image::storage_layout_oracle_shared.PNG[스토리지 레이아웃 Oracle Shared]
