...
. 검토 및 * 추가 *. 이미지:: snapctr_svcs_credential_04-Azure.png["GUI에서 이 단계를 보여 주는 스크린샷"]
. 마켓플레이스 구독 * 을 자격 증명과 연결해야 할 수도 있습니다. 이미지:: snapctr_svcs_credential_05-Azure.png["GUI에서 이 단계를 보여 주는 스크린샷"]
...
+
image::snapctr_svcs_restore_01-azure.png[snapctr svcs restore 01 Azure 를 참조하십시오]

. 복원 지점 * 을 선택하고 * 대체 위치로 복원 * 을 선택합니다.
+
image::snapctr_svcs_clone_01-azure.png[snapctr svcs clone 01 Azure 를 참조하십시오]
...
+
image::snapctr_svcs_clone_05-azure.png[snapctr svcs clone 05 Azure 를 참조하십시오]

. 클론 복제된 데이터베이스가 BlueXP에 즉시 등록되었음을 나타내는 BlueXP * 애플리케이션 * 페이지에서 클론 복제된 데이터베이스를 검증합니다.
+
image::snapctr_svcs_clone_06-azure.png[snapctr svcs clone 06 Azure 를 참조하십시오]

. 복제된 데이터베이스가 예상대로 실행 중임을 나타내는 Oracle Azure VM에서 복제된 데이터베이스를 검증합니다.
+
image::snapctr_svcs_clone_07-azure.png[snapctr svcs clone 07 Azure 를 참조하십시오]
