...
• 고성능 스케일아웃 NetApp 스토리지의 지원을 받는 새로운 대용량 JupyterLab 작업 공간을 빠르게 프로비저닝합니다.
• 엔터프라이즈급 NetApp 스토리지로 지원되는 새로운 NVIDIA Triton Inference Server 인스턴스를 신속하게 프로비저닝할 수 있습니다.
• 실험이나 빠른 반복을 지원하기 위해 고용량 JupyterLab 작업 공간을 거의 동시에 복제합니다.
• 백업 및/또는 추적/기준선 설정을 위해 대용량 JupyterLab 작업 공간의 스냅샷을 거의 동시에 저장합니다.
• 용량, 클론 및 스냅샷 고성능 데이터 볼륨을 거의 동시에 제공합니다.
