...
summary: NVIDIA DGX 시스템 지원 NetApp AI Pod - 소프트웨어 구성요소 
---
= NVIDIA DGX 시스템 지원 NetApp AI Pod - 소프트웨어 구성요소
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
link:aipod_nv_hw_components.html["이전: ONTAP AI 하드웨어 구성요소"]
...
image:oai_BaseCommand.png["오류: 그래픽 이미지가 없습니다"]
...
image:oai_ngc.png["오류: 그래픽 이미지가 없습니다"]
...
* 성능 및 짧은 지연 시간: ONTAP은 RDMA 기반 NFS를 사용하는 NVIDIA GPUDirect Storage(GDS)를 지원하는 등, 최저 지연 시간으로 가장 높은 처리량을 제공합니다.
...
link:aipod_nv_architecture.html["다음은 NVIDIA DGX 시스템 기반 NetApp AI Pod - 아키텍처."]
