...
를 사용합니다 https://["NetApp DataOps 툴킷"^]에서 다음 단계를 완료합니다.
...
를 눌러 설정합니다 https://["NGC"^]에서 다음 단계를 완료합니다.
...
Linux 기반이 아닌 운영 체제는 을 참조하십시오 https://["여기"^].



== NVIDIA Riva 서버: 지원 센터 정서 분석

를 눌러 설정합니다 https://["NVIDIA Riva"^]에서 다음 단계를 완료합니다.
...
. Riva 클라이언트 내에서 오디오 처리 라이브러리( https://["FFmpeg"^])
+
....
apt-get install ffmpeg
....
. 를 시작합니다 https://["Jupyter를 선택합니다"^] 서버.
...
. 를 준비하고 활성화합니다 https://["가상 환경"^] TAO 툴킷을 참조하십시오.
. 를 설치합니다 https://["필수 패키지"^].
...
사용합니다 https://["Riva의 Tao 툴킷 모델"^]에서 다음 단계를 완료합니다.
