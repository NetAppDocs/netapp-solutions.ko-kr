...
+
HCX 버전 링크 이미지:gcpd-hcx-image2.png[GCVE 리소스에 대한 링크를 사용한 HCX 콘솔 액세스]를 클릭하거나 vSphere Management Network 탭에서 HCX FQDN을 클릭하여 HCX 콘솔에 로그인할 수 있습니다. 이미지:: gcpd-hcx-image3.png[FQDN 링크를 사용한 HCX 콘솔 액세스]

. HCX Cloud Manager에서 * 관리 > 시스템 업데이트 * 로 이동합니다.
. 다운로드 요청 링크 * 를 클릭하고 OVA 파일을 다운로드합니다. 이미지: gcpd-hcx-image4.png [다운로드 링크 요청]
...
. VMware Engine 포털에서 리소스를 클릭하고 프라이빗 클라우드를 선택한 다음 * HCX Manager Cloud 버전 * 에서 다운로드 아이콘을 클릭합니다. 이미지:: gcpd-hcx-image6.png[HCX 라이센스 다운로드] 다운로드한 파일을 열고 라이센스 키 문자열을 복사합니다.
...
+
image::gcpd-hcx-image15.png[gcpd hcx image15 를 참조하십시오]

. 가상 컴퓨터 마이그레이션 마법사에서 원격 사이트 연결(대상 GCVE)을 선택합니다.
+
image::gcpd-hcx-image16.png[gcpd hcx 이미지16]

. 필수 필드(클러스터, 스토리지 및 대상 네트워크)를 업데이트하고 검증 을 클릭합니다.
+
image::gcpd-hcx-image17.png[gcpd hcx 이미지17]
...
+
image::gcpd-hcx-image18.png[gcpd hcx 이미지18]
