...
[%collapsible]
====
VM이 실행되는 동안 실수로 볼륨을 삭제하지 않으려면 아래 스크린샷과 같이 볼륨을 삭제할 수 없는 볼륨으로 표시해야 합니다. 이미지: GCP_ncvs_ds02.png[NetApp CVS non-deletable option] 자세한 내용은 문서를 참조하십시오. link:https://cloud.google.com/architecture/partners/netapp-cloud-volumes/creating-nfs-volumes#creating_an_nfs_volume["NFS 볼륨 생성 중"]
