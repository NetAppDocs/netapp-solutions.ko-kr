...
NetApp ONTAP용 Amazon FSx는 널리 사용되는 NetApp ONTAP 파일 시스템에 구축된 매우 안정적이고 확장 가능하며 고성능의 풍부한 기능 파일 스토리지를 제공하는 완전 관리형 서비스입니다. Amazon FSx for NetApp ONTAP(Multi-AZ)는 가용성 영역 수준 장애 발생 시 NAS 트래픽에 페일오버 기능을 지원하는 부동 IP 주소를 사용합니다. 이 IP 주소는 VPC CIDR 주소 공간 외부에 있으므로 ENI를 통해 SDDC로 라우팅할 수 없습니다. 따라서 VMware Transit Connect를 사용하여 NAS 인터페이스의 부동 IP 주소에 연결해야 합니다.
