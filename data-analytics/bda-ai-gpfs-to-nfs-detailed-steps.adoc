...
summary: 이 섹션은 GPFS를 구성하고 NetApp XCP를 사용하여 NFS로 데이터를 이동하는 데 필요한 세부 단계를 제공합니다. 
---
= GPFS에서 NFS로 - 자세한 단계
:hardbreaks:
:allow-uri-read: 
...
link:bda-ai-business-benefits.html["이전: 비즈니스 혜택."]
...
. 서버 중 하나에서 Linux용 Spectrum Scale 데이터 액세스를 다운로드하고 설치합니다.
+
....
...
…
....
. 모든 노드에 필수 구성 요소 패키지(셰프 및 커널 헤더 포함)를 설치합니다.
+
....
...
[root@mastr-51 installer]#
....
. 클러스터 정의 파일에 관리 노드 및 GPFS 노드를 추가합니다.
+
....
...
....
. GPFS에서 사용할 원격 셸 바이너리를 지정하고 '-r 인수'를 사용합니다.
+
....
...
[root@mastr-51 installer]#
....
. GPFS에서 사용할 원격 파일 복사 바이너리를 지정하고 '-rc 인수'를 사용하십시오.
+
....
...
[root@mastr-51 installer]#
....
. 모든 GPFS 노드에 설정할 포트 범위를 지정하고, '-e argument'를 사용한다.
+
....
...
[root@hdp2 gpfstest]#
....
. XCP를 사용하여 GPFS로 내보낸 NFS에서 NetApp NFS로 데이터를 이동하십시오.
+
....
...
link:bda-ai-mapr-fs-to-ontap-nfs.html["다음: MapR-FS에서 ONTAP NFS로"]
