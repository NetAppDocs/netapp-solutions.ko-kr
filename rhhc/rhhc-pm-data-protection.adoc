...
NOTE: 응용 프로그램 포드가 실행되고 있지 않으면 보안 컨텍스트 제약 때문에 발생한 오류가 있을 수 있습니다.
image:rhhc-scc-error.png[""]
을 편집하여 오류를 수정하십시오 `runAsUser` 및 `fsGroup` 의 필드 `statefuleset.apps/postgresql` 의 출력에 있는 uid 를 가진 개체입니다 `oc get project` 명령을 사용합니다.
