...
전원 공급 Astra Control(ACS 또는 ACC)은 NetApp Trident입니다. Trident는 Red Hat OpenShift, EKS, AKS, SUSE Rancher, Anthos 등과 같은 다양한 유형의 Kubernetes 클러스터를 FAS/AFF, ONTAP Select, CVO, Google Cloud Volumes Service, Azure NetApp Files, Amazon FSx ONTAP 등의 다양한 NetApp ONTAP 스토리지 버전과 통합합니다.
