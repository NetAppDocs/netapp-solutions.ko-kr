...
. 클러스터의 운영자 허브로 이동하여 Red Hat OADP 연산자를 선택합니다. 설치 페이지에서 모든 기본 선택 항목을 사용하고 설치를 클릭합니다. 다음 페이지에서 모든 기본값을 사용하고 Install(설치) 을 클릭합니다. OADP 연산자는 OpenShift-ADP라는 네임스페이스에 설치됩니다.
...
=== ONTAP S3를 사용한 Velero 구성 사전 요구 사항 세부 정보:
...
* S3에 액세스하는 데 사용할 수 있는 논리 인터페이스(LIF) IP입니다
...
* 먼저 ONTAP S3 사용자 자격 증명에 대한 암호를 생성합니다. 나중에 Velero를 구성하는 데 사용됩니다. CLI 또는 웹 콘솔에서 암호를 생성할 수 있습니다.
웹 콘솔에서 암호를 생성하려면 비밀 을 선택한 다음 키/값 비밀 을 클릭합니다.
그림과 같이 자격 증명 이름, 키 및 값을 입력합니다. S3 사용자의 액세스 키 ID와 비밀 액세스 키를 사용해야 합니다.


image::redhat_openshift_OADP_install_image4.jpg[ONTAP S3 자격 증명의 암호]

image::redhat_openshift_OADP_install_image5.jpg[ONTAP S3 자격 증명에 대한 암호를 생성합니다]


NOTE: CLI에서 클라우드 자격 증명이라는 기본 암호를 생성하려면 다음 명령을 사용할 수 있습니다. 백업 및 스냅샷 위치에서 동일한 자격 증명을 사용하는 경우 위와 같이 기본 암호를 생성하면 됩니다. 다른 시나리오는 OADP 설명서를 참조하십시오.

image::redhat_openshift_OADP_install_image6.jpg[CLI를 사용하여 ONTAP S3 자격 증명에 대한 암호를 생성합니다]

* 그런 다음 Velero를 구성하려면 Operators 아래의 메뉴 항목에서 Installed Operators 를 선택하고 OADP operator 를 클릭한 다음 DataProtectionApplication 탭을 선택합니다.
...
이제 YAML 보기로 이동하여 기본 정보를 바꾸거나 아래의 YAML 파일에 표시된 정보를 추가합니다.
...
spec:
  ...
  snapshotLocations:
    - velero:
        ...
        provider: aws
....
위의 YAML은 사양에서 예와 유사하게 적절히 구성해야 하는 다음과 같은 섹션을 포함하고 있습니다.

**backupLocations**
ONTAP S3(YAML에 표시된 자격 증명 및 기타 정보 포함)는 velero의 기본 BackupLocation으로 구성됩니다.

** 스냅샷 위치**
ONTAP S3는 Velero용 PVC 스냅샷의 기본 위치로 구성됩니다.
...
VolumeSnapshot 개체가 삭제된 경우에도 스냅샷이 유지될 수 있는지 확인하십시오. 이 작업은 deletionPolicy를 Retain으로 설정하여 수행할 수 있습니다. 그렇지 않은 경우 네임스페이스를 삭제하면 해당 네임스페이스에 백업된 모든 PVC가 완전히 손실됩니다.
