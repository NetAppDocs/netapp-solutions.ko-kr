...
이 솔루션은 VMware vSphere에 구축된 가상 어플라이언스를 사용합니다. F5 Big-IP 가상 어플라이언스에 대한 네트워킹은 네트워크 환경에 따라 2개 또는 3개 설정 구성으로 구성할 수 있습니다. 이 문서의 배포는 두 가지 설정 구성을 기반으로 합니다. Anthos에서 사용할 가상 어플라이언스 구성에 대한 자세한 내용은 을 참조하십시오 https://["여기"].
...
. F5에서 가상 응용 프로그램 OVA(Open Virtual Appliance) 파일을 다운로드합니다 https://["여기"].
+
