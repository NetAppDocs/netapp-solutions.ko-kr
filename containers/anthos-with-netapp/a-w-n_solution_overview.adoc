...
이 참조 문서는 여러 데이터 센터 환경에 배포될 NetApp 및 엔지니어링 파트너가 NetApp Anthos with NetApp 솔루션의 배포를 검증합니다.

또한, 영구 스토리지 관리를 위한 Astra Trident 스토리지 오케스트레이터를 사용하여 NetApp 스토리지 시스템과의 스토리지 통합에 대해서도 자세히 설명합니다. 마지막으로, 다양한 솔루션 검증 및 실제 사용 사례를 살펴보고 문서화합니다.
...
* Bare Metal의 제공된 'bmctl' 도구 또는 VMware vSphere의 'gkectl' 도구를 사용하여 배포된 Anthos 환경을 쉽게 배포 및 관리할 수 있습니다.
...
NetApp 솔루션의 Anthos는 이러한 당면 과제를 인정하며 고객이 선택한 데이터 센터 환경에서 Premise에 Anthos의 완전 자동화된 배포를 구현하여 각 문제를 해결하는 솔루션을 제시합니다.
...
| VMware 기반 Anthos 클러스터 | 컨테이너 오케스트레이션 | 1.10 
...
|===
link:a-w-n_overview_anthos.html["Next:Anthos 개요:NetApp의 Anthos"]
