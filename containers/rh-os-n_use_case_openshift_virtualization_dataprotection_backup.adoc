...
---
= OpenShift 가상화에서 VM에 대한 주문형 백업 생성
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
디스크를 지원하는 영구 볼륨의 스냅숏이 CSI에 의해 생성되고 YAML에 제공된 개체 저장소 위치로 이동합니다. 백업은 TTL에 지정된 대로 30일 동안 시스템에 유지됩니다.
...
== OpenShift 가상화에서 VM에 대한 예약된 백업 생성

일정에 따라 백업을 생성하려면 사용자 지정 리소스 예약을 생성해야 합니다.
...
백업에 포함할 네임스페이스와 백업에 대한 스토리지 위치도 지정됩니다. 따라서 Backup CR 대신 Schedule CR을 사용하여 지정된 시간과 빈도로 백업을 생성합니다.
