...
summary: NetApp에는 컨테이너식 애플리케이션에 대한 데이터를 프로비저닝, 보호 및 관리하는 Trident 및 Trident Protect로 검증된 여러 스토리지 플랫폼이 있어 DevOps 처리량을 정의하고 극대화하는 데 도움이 됩니다. 
---
= NetApp 스토리지 시스템 개요
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
NetApp에는 컨테이너식 애플리케이션에 대한 데이터를 프로비저닝, 보호 및 관리하는 Trident 및 Trident Protect로 검증된 여러 스토리지 플랫폼이 있어 DevOps 처리량을 정의하고 극대화하는 데 도움이 됩니다.
